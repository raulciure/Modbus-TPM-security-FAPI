...
def main():
    ...
    while not exit_flag:
        ...
        if args.measure_perf:
            ...
            sym_key =  latency_meter.measure_latency(lambda: DH_key_exchange(source_socket))   # for server gateway use 'source_socket' | for client gateway use 'dest_socket'
            latency_measure.export_to_file_key_exchange(latency_meter.get_average_latency(), latency_meter.get_average_runs())
        else:
            # do key exchange here
            sym_key = DH_key_exchange(source_socket)                                           # for server gateway use 'source_socket' | for client gateway use 'dest_socket'
